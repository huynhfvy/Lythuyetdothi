...
namespace CuoiKi
{
    internal class GraphicsTools
    {
        ...
        {
            if (ptStart == ptEnd) // vẽ khuyên
            {
                g.DrawEllipse(pLine,ptEnd.X, ptEnd.Y,25,25);
            }
            else
            {
                // them dau mui ten
                pLine.CustomEndCap = new AdjustableArrowCap(3, 10);
        ...
        public Bitmap CreateGraphics(List<List<int>> adjacent, List<Point> lstPointVertex)
        {
            ...
            for (int index = 0; index < adjacent.Count; ++index)
            {
                ...
                this.DrawNode(g, cbrush, pCircle, rect, pt, (index).ToString());
            }
            g.Dispose();
            pLine.Dispose();
            cbrush.Dispose();
